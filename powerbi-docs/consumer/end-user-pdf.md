...
* Экспорт отчетов Power BI, в которых больше 50 страниц, пока не поддерживается. На отчеты с разбивкой на страницы это ограничение не распространяется. Подробные сведения см. в разделе [Печать отчета с разбивкой на страницы](end-user-paginated-report.md#interact-with-a-paginated-report). 
* Экспорт отчетов, размер которых превышает 500 МБ, пока не поддерживается. 
