...
Значения нескольких строк, сгруппированные вместе в качестве входных данных условий, чтобы сформировать более показательное значение или измерение. Статистическая обработка возможна только для неявных мер (см. определение ниже).
...
**Емкость**    
[Power BI Premium]. Модели данных, работающие на оборудовании, полностью управляемом Майкрософт, в облачных центрах обработки данных Майкрософт для обеспечения непрерывной производительности в требуемом масштабе. Решения бизнес-аналитики доставляются всей организации независимо от лицензии Power BI.
...
Панель мониторинга Power BI — это отдельная страница в службе (часто называется полотном), на которой данные отображаются в виде визуализаций. Будучи ограниченной одной страницей, продуманная панель мониторинга содержит только наиболее важные элементы этого сообщения. Панели мониторинга можно создавать и просматривать только в службе Power BI, а не в Power BI Desktop. Дополнительные сведения см. в разделе [Основные понятия, панели мониторинга](end-user-basic-concepts.md).
...
**Явные меры**    
Power BI использует явные меры и неявные меры (см. определение ниже). Явные меры создаются конструкторами отчетов и сохраняются вместе с набором данных. Они отображаются в Power BI как поля, поэтому их можно использовать повторно. Например, конструктор отчетов создает явную меру *TotalInvoice*, которая суммирует все суммы счетов. Коллеги, использующие этот набор данных и имеющие доступ на внесение изменений в отчет, могут выбрать это поле и использовать его для создания визуального элемента. При добавлении или перетаскивании явной меры на холст отчета служба Power BI не применяет статистическую обработку.  Для создания явных мер требуется доступ на изменение набора данных. 
...
**Неявные меры**    
Power BI использует неявные меры и явные меры (см. определение выше). Неявные меры создаются динамически.  Например, при перетаскивании поля на холст отчета для создания визуального элемента. Power BI автоматически выполняет статистическую обработку значения с помощью одного из встроенных стандартных агрегатов (SUM, COUNT, MIN, AVG и т. д.). Для создания неявных мер требуется доступ на изменение отчета. 
...
**Power BI Premium**  .  
Дополнение к лицензии Power BI Pro, позволяющее организациям предсказуемо масштабировать решения бизнес-аналитики через приобретение зарезервированного оборудования в облаке Майкрософт. См. *account* (учетная запись).
...
Рабочая область в емкости, которая обозначена значком в виде ромба.  
