...
ms.locfileid: "97979983"
---
# <a name="private-links-for-accessing-power-bi"></a>Приватные каналы для доступа к Power BI

Сети Azure предоставляют компонент Приватный канал Azure, который позволяет Power BI обеспечить безопасный доступ через частные конечные точки Сети Azure. Благодаря Приватным каналам и Частным конечным точкам Azure трафик данных отправляется частным образом с помощью инфраструктуры магистрали корпорации Майкрософт, и поэтому данные не проходят через Интернет. 

Приватные каналы гарантируют, что пользователи Power BI будут использовать магистраль частной сети (Майкрософт) при переходе к ресурсам в службе Power BI.

Узнайте больше о [Приватных каналах Azure](https://azure.microsoft.com/services/private-link/).

## <a name="understanding-private-links"></a>Общие сведения о приватных каналах

Приватные каналы гарантируют, что трафик, поступающий *в* артефакты вашей организации Power BI (например, отчеты или рабочие области), всегда будет соответствовать настроенному для организации сетевому пути приватного канала. Пользовательский трафик к артефактам Power BI должен поступать из установленного приватного канала, и можно настроить Power BI на запрет всех запросов, поступающих не из настроенного сетевого пути. 

Приватные каналы *не* гарантируют защиту трафика от Power BI к внешним источникам данных в облаке или локальной среде. Вместо этого необходимо настроить правила брандмауэра и виртуальные сети, которые дополнительно защищают источники данных. 

### <a name="power-bi-and-private-links-integration"></a>Интеграция Power BI и приватных каналов

Частная конечная точка Azure для Power BI — это сетевой интерфейс, который надежно и безопасно подключается к службе Power BI, на базе Приватного канала Azure.   

Интеграция частных конечных точек позволяет развертывать службы "платформа как услуга" (PaaS) и предоставлять доступ к ним из виртуальных и локальных сетей клиента, при этом служба по-прежнему работает вне сети клиента. Частные конечные точки — это единая направленная технология, которая позволяет клиентам инициировать подключения к определенной службе, но не позволяет службе инициировать подключение к клиентской сети. Этот шаблон интеграции с Частной конечной точкой обеспечивает изоляцию управления, так как служба может работать независимо от конфигурации политики сети клиента. Для служб с несколькими клиентами эта модель Частных конечных точек предоставляет идентификаторы ссылок для предотвращения доступа к ресурсам других клиентов, размещенным в той же службе. При использовании Частных конечных точек из служб, использующих интеграцию, можно получить доступ только к ограниченному набору других ресурсов службы PaaS.  

Служба Power BI реализует частные конечные точки, а не конечные точки службы.  

Использование приватных каналов с Power BI предоставляет следующие преимущества:

1. Приватные каналы обеспечивают передачу трафика по магистрали Azure в частную конечную точку для облачных ресурсов Azure. 

2. Для изоляции сетевого трафика от инфраструктуры не на основе Azure, например локального доступа, клиенты должны будут настроить ExpressRoute или виртуальную частную сеть (VPN).  

## <a name="using-secure-private-links-to-access-power-bi"></a>Использование защищенных приватных каналов для доступа к Power BI
...
5. В окне **Создание виртуальной машины — сети** выберите следующую информацию.
    ...
    |Пространство адресов| Оставьте значение по умолчанию **10.5.0.0/24**.|
    |Подсеть |Оставьте значение по умолчанию **mySubnet (10.5.0.0/24)** .|
...
* Использование внешних изображений или тем недоступно при использовании среды с приватными каналами.
...
* Метрики использования *не* работают, если включены Приватные каналы.
* Публикация в Интернете не поддерживается (и недоступна для выбора) при включении параметра **Блокировать общедоступный Интернет** в Power BI.
