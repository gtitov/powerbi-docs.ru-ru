...
#### <a name="updates-for-premium-gen2-preview"></a>Обновления для Premium 2-го поколения (предварительная версия)
Для Premium 2-го поколения не используются совокупные ограничения памяти, поэтому обновлению всегда предоставляется достаточно памяти. Кроме того, не существует ограничений на 1,5x обновлений, выполняющихся на каждом виртуальном ядре. Вы можете запланировать и запустить столько обновлений, сколько требуется в любой момент времени, а служба Power BI будут выполнять эти обновления в запланированное время.
