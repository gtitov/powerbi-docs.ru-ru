...
2.  Если вы используете XMLA в Premium 2-го поколения, убедитесь, что используете последние версии [средств моделирования данных и управления ими](service-premium-connect-tools.md#data-modeling-and-management-tools). 

3.  Функции служб Analysis Services в Premium 2-го поколения поддерживаются только в последних клиентских библиотеках. Предполагаемые даты выпуска зависимых средств, поддерживающих это требование:
...
При использовании **Premium 2-го поколения** и [Embedded 2-го поколения](../developer/embedded/power-bi-embedded-generation-2.md) объем памяти, доступный на каждом узле, равен значению для ограничения объема памяти одного артефакта, а не совокупному потреблению памяти. Например, в Premium 2-го поколения емкости P1 размер одного набора данных ограничен 25 ГБ. Для сравнения, в Premium общая занимаемая память одновременной обработки наборов данных не должна превышать 25 ГБ.
...
Premium 2-го поколения и [Embedded 2-го поколения](../developer/embedded/power-bi-embedded-generation-2.md) не требуют ограничений по совокупной памяти, поэтому одновременные обновления наборов данных не влияют на ограничения ресурсов. Не существует ограничений на обновления, выполняющиеся на каждом виртуальном ядре. Однако обновление отдельных наборов данных по-прежнему регулируется существующими объемами памяти и ограничениями ЦП. Вы можете запланировать и запустить столько обновлений, сколько требуется в любой момент времени, а служба Power BI будут выполнять эти обновления в запланированное время.
...
> [!NOTE]
> В **Premium 2-го поколения (предварительная версия)** и [Embedded 2-го поколения](../developer/embedded/power-bi-embedded-generation-2.md) управление памятью для отчетов с разбиением на страницы отсутствует. При использовании Premium 2-го поколения и Embedded 2-го поколения отчеты с разбиением на страницы поддерживаются в SKU EM1–EM3 и A1–A3.
