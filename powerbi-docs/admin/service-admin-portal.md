...
На портале администрирования можно управлять параметрами Power BI для вашей организации. Портал включает метрики использования, доступ к Центру администрирования Microsoft 365 и параметры, которые управляют Power BI для всех пользователей.
...
1. В заголовке страницы выберите **Настройки** > **Портал администрирования**.
   ...
   :::image type="content" source="media/service-admin-portal/settings-portal.png" alt-text="Меню параметров с выбранным порталом администрирования.":::

На портале администрирования есть несколько разделов. Сведения о каждом из них представлены ниже.

   :::image type="content" source="media/service-admin-portal/portal-menu.png" alt-text="Меню портала администрирования.":::

* [Метрики использования](#usage-metrics)
* [Пользователи](#users)
* [Premium на пользователя (предварительная версия)](#premium-per-user-preview)
...
## <a name="premium-per-user-preview"></a>Premium на пользователя (предварительная версия)

Premium на пользователя — это новый способ лицензирования функций уровня Premium отдельно для каждого пользователя. Эта функция в настоящее время находится на стадии предварительной версии. После того как хотя бы одному пользователю назначена лицензия Premium на пользователя, связанные функции можно включить в любой рабочей области. Администраторы могут управлять параметрами автоматического обновления и рабочей нагрузки набора данных, которые отображаются пользователям, и их значениями по умолчанию. Например, доступ к конечной точке XMLA можно отключить, настроить в режиме "только для чтения" или настроить для чтения и записи.

   :::image type="content" source="media/service-admin-portal/premium-per-user-options.png" alt-text="Параметры Premium на пользователя.":::

Чтобы получить дополнительные сведения об этой модели лицензирования, см. статью [Часто задаваемые вопросы о Power BI Premium на пользователя (предварительная версия)](service-premium-per-user-faq.md).
...
В следующих разделах описывается несколько параметров на вкладке **Параметры клиента**.

> [!NOTE]
> Для вступления измененных параметров в силу для всех пользователей в организации может потребоваться до 15 минут.

В основном параметры могут иметь одно из трех состояний:
...
### <a name="allow-users-to-try-power-bi-paid-features"></a>Возможность пробного использования платных функций Power BI
...
### <a name="show-a-custom-message-before-publishing-reports"></a>Отображение настраиваемого сообщения перед публикацией отчетов  

Администраторы могут создать настраиваемое сообщение, отображаемое перед публикацией отчета пользователем из Power BI Desktop. После включения параметра необходимо указать текст **настраиваемого сообщения**. Настраиваемое сообщение может быть обычным текстом или следовать синтаксису Markdown, как показано в следующем примере сообщения:
...
Текстовое поле **Настраиваемое сообщение** не поддерживает прокрутку, поэтому можно указать сообщение длиной до 5000 символов.

:::image type="content" source="media/service-admin-portal/admin-show-custom-message.png" alt-text="Снимок экрана: поле настраиваемого сообщения.":::

Когда пользователи публикуют отчеты в рабочих областях в Power BI, для них отображается созданное вами сообщение.

:::image type="content" source="media/service-admin-portal/admin-user-show-custom-message.png" alt-text="Диалоговое окно, которое пользователи видят при публикации отчетов в рабочей области.":::

Как и в случае с другими параметрами клиента, можно указать, в отношении кого применяется **настраиваемое сообщение**:

- **Вся организация**.
- **Конкретные группы безопасности**.
- Или **За исключением отдельных групп безопасности**.
...
|Параметр **Коды внедрения** на портале администратора|Статус может иметь одно из следующих значений.<br>* Активен<br>* Не поддерживается<br>* Заблокирован|Отображается состояние **Отключено**|Статус может иметь одно из следующих значений.<br>* Активен<br>* Не поддерживается<br>* Заблокирован<br><br>Если пользователь не авторизовался в соответствии с настройками клиента, отображается состояние **Нарушено**.|
|Существующие опубликованные отчеты|Все включены|Все отключены|Отчеты продолжают отображаться для всех.|

### <a name="copy-and-paste-visuals"></a>Копирование и вставка визуальных элементов

Пользователи в организации могут копировать визуальные элементы из плитки или визуального элемента отчета и вставлять их как статические изображения во внешние приложения.

![Снимок экрана: включение параметра копирования и вставки визуальных элементов.](media/service-admin-portal/powerbi-admin-portal-copy-paste-visuals-setting.png)
...
![Снимок экрана: экспорт в Excel](media/service-admin-portal/powerbi-admin-portal-export-to-excel-setting.png)

### <a name="export-to-csv"></a>Экспорт в CSV

Пользователи в организации могут экспортировать данные из плитки, визуализации или отчета с разбивкой на страницы в CSV-файл.

![Снимок экрана: параметр экспорта в CSV-файл](media/service-admin-portal/powerbi-admin-portal-export-to-csv-setting.png)

### <a name="download-reports"></a>Скачивание отчетов

Пользователи в организации могут скачивать PBIX-файлы и отчеты с разбивкой на страницы.

![Снимок экрана: скачивание отчетов.](media/service-admin-portal/powerbi-admin-portal-download-reports-setting.png)

### <a name="allow-live-connections"></a>Разрешение динамических подключений

Пользователи организации могут использовать активное подключение (Live Connect) к службе Power BI. Разрешение динамических подключений также позволяет пользователям выполнять анализ в Excel.

![Снимок экрана: параметр "Разрешить динамические подключения".](media/service-admin-portal/powerbi-admin-portal-allow-live-connections-setting.png)
...
Пользователи в организации могут экспортировать отчеты Power BI в виде файлов PowerPoint или PDF-документов.

![Снимок экрана: экспорт отчетов в виде документов PowerPoint или PDF.](media/service-admin-portal/powerbi-admin-portal-export-pptx-pdf-setting.png)

### <a name="export-reports-as-mhtml-documents"></a>Экспорт отчетов в виде документов MHTML

Пользователи в организации могут экспортировать отчеты с разбивкой на страницы как документы MHTML.

![Снимок экрана: экспорт в MHTML.](media/service-admin-portal/powerbi-admin-portal-export-mhtml-setting.png)

### <a name="export-reports-as-word-documents"></a>Экспорт отчетов в виде документов Word

Пользователи в организации могут экспортировать отчеты с разбивкой на страницы в виде документов Word.

![Снимок экрана: экспорт в Word.](media/service-admin-portal/powerbi-admin-portal-export-word-setting.png)

### <a name="export-reports-as-xml-documents"></a>Экспорт отчетов в виде XML-документов

Пользователи в организации могут экспортировать отчеты с разбивкой на страницы в виде XML-документов.

![Снимок экрана: экспорт в XML.](media/service-admin-portal/powerbi-admin-portal-export-xml-setting.png)

### <a name="export-reports-as-image-files-preview"></a>Экспорт отчетов в файлы изображений (предварительная версия)

Пользователи в организации могут использовать API экспорта отчета в виде файлов изображений.

![Снимок экрана: экспорт как изображения.](media/service-admin-portal/powerbi-admin-portal-export-as-image-setting.png)

### <a name="print-dashboards-and-reports"></a>Печать панелей мониторинга и отчетов


![Снимок экрана: панель мониторинга печати и параметры отчетов.](media/service-admin-portal/powerbi-admin-portal-print-dashboards-reports-setting.png)

### <a name="certification"></a>Сертификация
Разрешение пользователям организации удостоверять наборы и потоки данных, а также приложения. Дополнительные сведения см. в разделе [Реализация сертификации содержимого](service-admin-setup-certification.md).

### <a name="email-subscriptions"></a>Почтовые подписки
Пользователи в организации могут создавать почтовые подписки. Дополнительные сведения о [подписках](../collaborate-share/service-report-subscribe.md)см. здесь.
...
### <a name="allow-connections-to-featured-tables"></a>Разрешение подключений к рекомендуемым таблицам

С помощью этой функции администраторы Power BI могут контролировать использование рекомендуемых таблиц в коллекции "Типы данных" Excel пользователями организации. 

![Снимок экрана: параметр "Разрешить подключения к рекомендуемым таблицам".](media/service-admin-portal/powerbi-admin-portal-allow-connections-featured-tables-setting.png)

>[!NOTE]
>Подключения к рекомендуемым таблицам также отключаются, если для параметра [Разрешить активные подключения](#allow-live-connections) задано значение "Отключено".

См. дополнительные сведения о [рекомендуемых таблицах Power BI в Excel](../collaborate-share/service-excel-featured-tables.md).

### <a name="share-to-teams"></a>Отправить в Teams

Этот параметр позволяет организациям скрывать кнопку **Поделиться в Teams** в службе Power BI. Если этот параметр отключен, пользователи не увидят кнопки **Поделиться в Teams** на панели действий или в контекстных меню при просмотре отчетов и панелей мониторинга в службе Power BI.
...
### <a name="allow-xmla-endpoints-and-analyze-in-excel-with-on-premises-datasets"></a>Активация конечных точек XMLA и Анализа в Excel для локальных наборов данных

Пользователи в организации могут просматривать локальные наборы данных Power BI и взаимодействовать с ними в Excel. Это также позволяет подключаться к конечным точкам XMLA. [Дополнительные сведения](../collaborate-share/service-analyze-in-excel.md)
...
### <a name="web-content-on-dashboard-tiles"></a>Веб-содержимое на плитках панели мониторинга

Пользователи в организации могут добавлять и просматривать плитки веб-содержимого на панелях мониторинга Power BI. [Дополнительные сведения](../create-reports/service-dashboard-add-widget.md)

> [!NOTE]
> Это может привести к угрозам безопасности из-за вредоносного веб-содержимого.
...
Пользователи в организации могут создавать и использовать потоки данных. Общие сведения о потоках данных. [Самостоятельная подготовка данных в Power BI](../transform-model/dataflows/dataflows-introduction-self-service.md). Чтобы включить потоки данных в емкости Premium, см. [Настройка рабочих нагрузок](service-admin-premium-workloads.md).
...
- Обновление интерфейса классических рабочих областей
...
Администраторы могут обновить интерфейс классических рабочих областей. Администраторы могут выбрать одну или несколько рабочих областей с типом **Группа** для обновления. Обновления помещаются в очередь и выполняются асинхронно. Выполнение всех **ожидающих** обновлений может занять от нескольких минут до нескольких дней, поскольку общая частота обновлений, инициированных администратором, ограничена, чтобы обеспечить бесперебойную работу службы. С помощью столбца **Состояние обновления рабочей области** администраторы могут отслеживать ход выполнения обновлений, инициированных администратором. Администраторы могут отменить инициированные администратором обновления, если они **ожидающие**. Чтобы немедленно обновить рабочую область, обратитесь к администратору рабочей области и запустите обновление в области "Параметры" рабочей области. [Подробнее об обновлении рабочей области перед началом обновления рабочей области, инициированной администратором Power BI.](../collaborate-share/service-upgrade-workspaces.md).

В следующей таблице приводятся более подробные сведения о состоянии обновления.

|Status  |Описание  |
|---------|---------|
| **(Пусто)** | Рабочая область не обновляется администратором Power BI. |
| **Ожидание** | Рабочая область поставлена в очередь для обновления. Обновление можно отменить. |
| **Выполняется** | Рабочая область обновляется. Отменить обновление невозможно. |
| **Завершено** | Рабочая область обновлена в течение последних 30 дней администратором Power BI. Администратор рабочей области может вернуться к классическому варианту, если это необходимо, в течение 30 дней после обновления рабочей области. |
