...
:::image type="content" source="media/desktop-relationships-understand/animation-filter-propagation.gif" alt-text="Анимированный пример распространения фильтров связей.":::
...
Тем не менее составная модель может включать в себя таблицы, использующие различные режимы хранения (импорт, DirectQuery или двойной) или несколько источников DirectQuery. Каждый источник, в том числе кэш Vertipaq данных импорта, считается _исходной группой_. Связи моделей могут быть _внутренними_ или _межэлементными_. Внутренняя связь — это связь двух таблиц в исходной группе, а связь между исходными группами — это связь таблиц из разных исходных групп. Обратите внимание, что связи в моделях импорта или DirectQuery всегда внутренние.

Рассмотрим пример составной модели.

:::image type="content" source="media/desktop-relationships-understand/source-group-example.png" alt-text="Пример составной модели из двух исходных групп.":::

В этом примере составная модель включает в себя две исходные группы: Vertipaq и DirectQuery. Исходная группа VertiPaq содержит три таблицы, а исходная группа DirectQuery содержит две таблицы. Есть одна межэлементная связь, позволяющая связать таблицу в группе Vertipaq с таблицей в группе DirectQuery.

### <a name="regular-relationships"></a>Обычные связи

Когда обработчик запросов может определить одну сторону связи, модель называется _обычной_. Это подтверждает то, что в столбце одной стороны содержатся уникальные значения. Все отношения типа "одно ко многим" внутри исходной группы являются обычными.

На примере ниже показаны две обычные связи, помеченные как **R**. Отношения включают в себя связь "одна ко многим", содержащуюся в исходной группе Vertipaq, и связь "одна ко многим", содержащуюся в исходной группе DirectQuery.

:::image type="content" source="media/desktop-relationships-understand/source-group-example-regular.png" alt-text="Пример составной модели из двух исходных групп, где отмечены обычные связи.":::
...
Расширение таблицы также происходит для связей типа "одна к одной" внутри исходной группы, но с использованием семантики FULL OUTER JOIN. Это гарантирует, что при необходимости пустые виртуальные строки добавятся с необходимой стороны.
...
:::image type="content" source="media/desktop-relationships-understand/animation-expanded-table.gif" alt-text="Анимированный пример расширения таблицы.":::
...
На примере ниже показаны две ограниченные связи, помеченные как **L**. Два отношения включают в себя связь "много ко многим", содержащуюся в исходной группе Vertipaq, и межэлементную связь "одна ко многим".

:::image type="content" source="media/desktop-relationships-understand/source-group-example-limited.png" alt-text="Пример составной модели из исходных групп, где отмечены ограниченные связи.":::
...
> [!NOTE]
> В представлении модели Power BI Desktop не всегда можно определить тип связи модели: обычная или ограниченная. Связь "много ко многим" всегда будет ограниченной, как и связь "одна ко многим", если она между исходными группами. Чтобы определить, является ли эта связь межэлементной, необходимо проверить режимы хранения таблиц и источники данных, чтобы у них было правильное определение.
...
1. Связь "один к одному" внутри исходной группы
