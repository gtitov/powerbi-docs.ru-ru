...
* Создание потоков данных выполняется в среде Power Query Online (PQO). См. ограничения, описанные в статье об [ограничениях, касающихся Power Query](/power-query/power-query-online-limits).
...
Дополнительные сведения о поддерживаемых интерфейсах REST API для потоков данных см. в [справочнике по REST API](/rest/api/power-bi/dataflows). Ниже приведены моменты, на которые следует обратить внимание.
