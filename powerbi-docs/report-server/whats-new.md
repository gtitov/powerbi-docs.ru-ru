...
description: Сведения о новых возможностях последней версии Сервера отчетов Power BI. В этой статье приводятся сведения об основных функциях. Материалы обновляются по мере выпуска новых версий.
author: maggiesMSFT
ms.author: maggies
ms.reviewer: ''
ms.service: powerbi
...
Узнайте о возможностях новых версий Сервера отчетов Power BI и версий Power BI Desktop для Сервера отчетов. В этой статье приводятся сведения об основных функциях. Материалы обновляются по мере появления новых выпусков. Дополнительные сведения о новых возможностях Сервера отчетов Power BI см. в статье [Журнал изменений для Сервера отчетов Power BI](changelog.md).

Скачайте [Сервер отчетов Power BI и Power BI Desktop для Сервера отчетов](https://powerbi.microsoft.com/report-server/).

## <a name="january-2021"></a>Январь 2021 г.

Ниже приведен список новых и обновленных функций и компонентов. Более подробные сведения см. в записи блога по [Серверу отчетов Power BI за январь 2021 года](https://powerbi.microsoft.com/blog/power-bi-report-server-january-2021-feature-summary/).
...
- Общедоступная версия функции "Применить все фильтры"
- Ползунок визуального масштабирования
- Проверка отзыва сертификата для веб-соединений
- Область выделения в представлении мобильного макета
- Поддержка ArcGIS для Power BI

**Подключение к данным**

- Общедоступная версия соединителя Hive LLAP
- Новые соединители: Actian, Anaplan, Starburst Presto

### <a name="new-connection-metadata-format-preview"></a>Новый формат метаданных подключения (предварительная версия)

В октябре мы изменили способ хранения метаданных подключения в формате PBIX-файла. Это изменение является частью долгосрочной инициативы по расширению возможностей программного доступа к файлам PBIX и их изменения. Мы уже включили это изменение в версию Power BI Desktop, которая выпускается ежемесячно согласно графику выпуска службы Power BI. Начиная с этого месяца изменение будет доступно в качестве предварительной версии в текущем выпуске Power BI Desktop для Сервера отчетов.

Начиная с этого выпуска, будет выводиться следующее уведомление, предлагающее включить предварительную версию и перейти на расширенный формат:

"Ожидается в ближайшее время: включите предварительную версию расширенных метаданных, чтобы повысить производительность и уровень безопасности в Power BI".

Следует отметить следующее.

- Существующие наборы данных будут работать по-прежнему даже без обновления. Они будут обновлены, когда расширенный формат станет общедоступным.
- Power BI Desktop для Сервера отчетов автоматически обновит существующий набор данных до расширенного формата. Мы не рекомендуем перезаписывать существующие отчеты на Сервере отчетов с использованием этой обновленной версии, если вам может потребоваться вернуться к предыдущему формату.
- Использовать предварительную версию не обязательно, хотя мы рекомендуем опробовать ее и предоставить нам отзыв. Мы планируем сделать расширенный формат общедоступным и обязательным в следующем выпуске Сервера отчетов.
 
Администратор может отключить эту предварительную версию, отменить переход на версию 3 и скрыть флажок предварительной версии функции. Задайте в реестре значение 0 для следующего параметра DWORD: AllowV3Models в
...
Дополнительные сведения см. в статье [Использование расширенных метаданных набора данных](../connect-data/desktop-enhanced-dataset-metadata.md).

### <a name="power-bi-report-server"></a>Cервере отчетов Power BI

**Прекращение поддержки некоторых браузеров**

Если вы используете устаревшую версию Edge или Internet Explorer 11, пора обновить браузер. Поддержка устаревшей версии Edge прекращается 9 марта 2021 г. Поддержка Internet Explorer 11 прекращается 17 августа 2021 г. 

На веб-портале Сервера отчетов Power BI появится предупреждение о том, что Internet Explorer 11 больше не поддерживается. Пора выполнить обновление! Подробные сведения см. в статье [Поддержка браузера для сервера отчетов Power BI](browser-support.md).

**Новый значок продукта**

Сервер отчетов Power BI получил новый значок в рамках обновления значков для всех продуктов Майкрософт.

:::image type="content" source="media/whats-new/power-bi-report-server-new-logo.png" alt-text="Новый логотип Сервера отчетов Power BI.":::

## <a name="october-2020"></a>Октябрь 2020 г.

Ниже приведен полный список новых и обновленных функций и компонентов. Более подробные сведения см. в записи блога по [Серверу отчетов Power BI за октябрь 2020 года](https://powerbi.microsoft.com/blog/power-bi-report-server-october-2020-feature-summary/). 
...
В Power BI Desktop для Сервера отчетов Power BI появилась современная лента. Мы будем вносить дополнения и добавлять функции для этой ленты. Дополнительные сведения об изменениях и преимуществах см. в разделе [Использование обновленной ленты в Power BI Desktop](../create-reports/desktop-ribbon.md).

:::image type="content" source="media/whats-new/report-server-new-ribbon.png" alt-text="Снимок экрана: новая лента в Power BI Desktop для Сервера отчетов Power BI.":::

**Иерархические срезы**

Если вы хотите отфильтровать несколько связанных полей в одном срезе, можете создать так называемый *иерархический срез*. Теперь, когда эта функция общедоступна, она приобрела преимущества в виде дополнительных возможностей.

- Новые параметры для значков развертывания и свертывания.
- Теперь значки масштабируются вместе с размером шрифта.
- Настраиваемый отступ для дочерних элементов.
- Дальнейшая настройка текста заголовка среза.

:::image type="content" source="media/whats-new/report-server-slicer-hierarchy.png" alt-text="Иерархический срез в Power BI Desktop для Сервера отчетов Power BI.":::

Дополнительные сведения см. в разделе [Срезы в Power BI](../visuals/power-bi-visualization-slicers.md) в документации.

**Улучшения мобильной разработки** 

В этом выпуске мы представляем улучшенную процедуру разработки мобильного макета и дополнительные возможности для работы с такими макетами. По мере того как все больше организаций применяют мобильные приложения Power BI, мы стали получать отзывы с запросами о дополнительных возможностях и улучшениях для процедуры мобильной разработки. Усовершенствования в этом выпуске помогут вам легче создавать привлекательные интерактивные отчеты, оптимизированные для мобильных устройств.

Ниже приведены новые возможности и функции для создания отчетов, оптимизированных для мобильных устройств.  

- Новый эмулятор телефона  
- Обновленная панель визуализации  
- Поддержка наложенных визуальных элементов  
- Закладки, доступные в представлении мобильного макета  
- Отключение линий сетки и привязки к сетке  

:::image type="content" source="media/whats-new/report-server-mobile-layout.png" alt-text="Расширенный мобильный макет в Power BI Desktop для Сервера отчетов Power BI.":::

Дополнительные сведения см. в этой записи блога [Улучшения мобильной разработки](https://powerbi.microsoft.com/blog/power-bi-desktop-june-2020-feature-summary/#_mobile).

**Подготовка данных: автоматическое обнаружение таблиц из файлов Excel**

Эта новая функция соединителя Excel автоматически определяет разделы каждой электронной таблицы Excel для извлечения в таблицу. Она отображает их в группе "Предлагаемые таблицы" в навигаторе. Ранее данные требовалось форматировать как таблицу или именованный диапазон в Excel. В противном случае пришлось бы извлекать соответствующие строки или столбцы с конкретными преобразованиями, такими как пропуск строки и удаление столбцов, из объектов листа, как на листе Sheet1 в следующем примере. Чтобы использовать эту функцию, необходимо включить ее в диалоговом окне **Параметры**.

:::image type="content" source="media/whats-new/report-server-excel-table.png" alt-text="Распознавание таблицы Excel в Power BI Desktop для Сервера отчетов Power BI.":::

**Моделирование: расширенные метаданные наборов данных**

Раньше с моделью данных сопоставлялись только загружаемые запросы, созданные в Power Query. Теперь все запросы сопоставляются с объектами в модели данных. Запросы создаются повторно на основе модели данных при открытии PBIX-файла.

Это обновление серверной части сделало возможными дальнейшие улучшения. 

Пример:

- С добавлением внешних средств в Power BI Desktop (в настоящее время находится на этапе предварительной версии) таблицы, добавляемые с их помощью, теперь отображаются в виде запросов при повторном открытии отчета.
- Если бы вы теперь попытались экспортировать файл PBIT и распаковать его, то увидели бы модель в формате JSON и больше не получали бы ошибки о поврежденных файлах. 

Если у вас имеется более старый PBIX-файл, который не может быть обновлен автоматически по таким причинам, как непримененные изменения, необходимо успешно обновить модель, прежде чем можно будет вносить другие изменения в моделирование. 

**Поддержка Windows 7 заканчивается**

14 января 2020 г. была прекращена поддержка Windows 7, продолжавшаяся 10 лет. В связи с этим мы прекратим поддержку Power BI Desktop в Windows 7 31 января 2021 г. После этого Power BI Desktop будет поддерживаться только в операционной системе Windows 8 и более поздних версий. Выпуск Power BI Desktop за январь 2021 г., оптимизированный для Сервера отчетов, будет поддерживаться в соответствии с современной политикой жизненного цикла. Иными словами, он полностью поддерживается до следующего выпуска (который сейчас запланирован на май 2021 г.). После мая 2021 г. он будет получать обновления для системы безопасности лишь до января 2022 г. Вся поддержка будет прекращена после января 2022 г. Дополнительные сведения см. в разделе [График поддержки Сервера отчетов Power BI](support-timeline.md). 

**Полный список обновлений**

Ниже приведен полный список обновлений в октябрьском выпуске Power BI Desktop для Сервера отчетов Power BI. 

**Отчеты**

- Современная лента
- Водяные знаки на холсте
- Определение общего количества меток визуальных элементов с накоплением
- Добавлен общий визуальный элемент для упорядочения слоев
- Условные обозначения градиента
- Фильтр относительного времени
- Возможность дальнейшей настройки текста заголовка среза
- Усовершенствования иерархических срезов

    - Новые параметры для значков развертывания и свертывания
    - Теперь значки масштабируются вместе с размером шрифта
    - Настройка отступа для дочерних элементов
...
- Улучшения мобильной разработки
    ...
    - Обновленная панель визуализации
    - Поддержка наложенных визуальных элементов
    - Закладка, доступная в представлении мобильного макета
    - Отключение линий сетки и привязки к сетке  

**Визуализации**

- Параметры форматирования точек на графиках 

**Моделирование**

- Расширенные метаданные наборов данных
- Улучшения производительности функций IF и SWITCH
- Поддержка финансовых функций Excel
- Представление моделей с поддержкой динамического подключения
- Обновления представления моделей 

**Подготовка данных**

- Автоматическое обнаружение таблиц из файлов Excel  
- Автоматическое обнаружение таблиц из файлов JSON  
- Глобальный параметр для отключения автоматического определения типа 

**Другое**  

- Экспорт источника данных в PBIDS в Power BI Desktop
- Отклонение экрана-заставки 

### <a name="power-bi-report-server"></a>Cервере отчетов Power BI

**Изменение строк подключения к источникам данных в отчетах Power BI с помощью PowerShell**

В выпуске Сервера отчетов Power BI за октябрь мы предоставляем возможность обновлять подключения для отчетов Power BI для DirectQuery и обновления. Эта возможность также является критическим изменением по сравнению с тем, как аналогичную настройку можно выполнить в предыдущих выпусках. Дополнительные сведения см. в разделе [Изменение строк подключения к источникам данных в отчетах Power BI с помощью PowerShell — Сервер отчетов Power BI](connect-data-source-apis.md). 
...
В выпуске Power BI Desktop для Сервера отчетов Power BI за август 2018 г. появилось множество новых функций. Они приведены ниже с группированием по областям:
...
В выпуске Power BI Desktop для Сервера отчетов Power BI за март 2018 г. появилось множество новых функций. Они приведены ниже с группированием по областям:
