...
- **Сервер отчетов Power BI**
    - *Версия: 1.10.7698.12811 (сборка 15.0.1105.194), дата выпуска: 21 января 2021 г.*
        - Компоненты
            - Новые значки для Power BI и отчетов Power BI.
            - В состав выпуска входит интерфейс API настраиваемых визуальных элементов версии 3.5.0.
            - В новой предварительной версии портала добавлен переключатель UsePortalV2 в таблице ConfigurationInfo каталога ReportServer.
        - Исправленные ошибки
            - Исправлена проблема с запланированным обновлением отчетов Power BI с определенными моделями, в которых используются расширенные метаданные модели.
            - Исправлена проблема при изменении управляемых данными подписок на портале.
        - Обновления для системы безопасности

- **Power BI Desktop (оптимизировано для решения "Сервер отчетов Power BI")** .
   - *Версия: 2.88.1382.0 (январь 2021 г.), дата выпуска: 21 января 2021 г.* (новая сборка и новая версия)
        - Содержит изменения, необходимые для подключения к Серверу отчетов Power BI (январь 2021 г.).        
   
## <a name="october-2020"></a>Октябрь 2020 г.
- **Сервер отчетов Power BI**
    - *Версия: 1.9.7675.15620 (сборка 15.0.1104.300), дата выпуска: 8 января 2021 г.*
        - Исправления ошибок
            - Исправлена проблема с обновлением отчетов с двумя или более источниками данных, которые отличаются только регистром букв.
            - Исправлена проблема с обновлением отчетов с определенными сочетаниями вложенных соединений.
    - *Версия: 1.9.7627.11028 (сборка 15.0.1104.264), дата выпуска: 18 ноября 2020 г.*
        - Исправленные ошибки
            - Исправлена проблема, из-за которой пользователи не могли внести изменения в поля в параметрах сайта с помощью портала.
            - Исправлена проблема с обновлением отчетов Power BI при использовании источника данных "EnterData".
            - Исправлена проблема с обновлением некоторых моделей с помощью метаданных расширенного набора данных.
            - Исправлена проблема, при которой некоторые отчеты Power BI не удавалось опубликовать на сервере отчетов.
    - *Версия: 1.9.7604.41261 (сборка 15.0.1104.239), дата выпуска: 27 октября 2020 г.*
         - Компоненты
            - Включена поддержка расширенных метаданных набора данных в Сервере отчетов Power BI.
            - Включена возможность обновления подключений для отчетов Power BI для DirectQuery и обновления (дополнительные сведения см. в разделе [Изменение строк подключения к источнику данных](./connect-data-source-apis.md)).
        - Обновления для системы безопасности
        - Исправления ошибок
            - Исправлена проблема, препятствующая изменению расписаний обновления отчетов Power BI пользователями.
            - Исправлена путаница с сообщениями об ошибках, которые пользователи, управляющие отчетами, получали при истекшем сроке действия учетных данных.
            - Исправлена проблема с экспортом отчетов с точками в имени.
            - Исправлены проблемы со средством чтения с экрана в табликсе.
            - Исправлена проблема с файлами журнала, которые в некоторых обстоятельствах оказывались пустыми.
            - Исправлена проблема с перезаписыванием файла Excel при передаче.
            - Исправлена ошибка метода REST API Model.UpdateCacheSnapshot.
            - Исправлена проблема с подключениями источника данных SAP BW через XMLA.
            - Исправлена проблема, когда диалоговое окно "Подключение к Power BI" не закрывается.
            - Исправлена проблема со значением по умолчанию для расширенного компонента CustomHeaders.
            - Обновлен отрисовщик MHTML для использования более нового HTML DOCTYPE.

- **Power BI Desktop (оптимизировано для решения "Сервер отчетов Power BI")** .
   - *Версия: 2.86.1321.0 (октябрь 2020 г.), дата выпуска: 18 ноября 2020 г.*
        - Исправления ошибок
   - *Версия: 2.86.961.0 (октябрь 2020 г.), дата выпуска: 27 октября 2020 г.* (новая сборка и новая версия).
        - Содержит изменения, необходимые для подключения к решению "Сервер отчетов Power BI" (октябрь 2020 г.).        
...
- **Сервер отчетов Power BI**
    ...
    - *Версия: 1.8.7450.37410 (сборка 15.0.1103.227), дата выпуска: 27 мая 2020 г.*
        ...
        - Исправления ошибок
            ...
            - Устранена проблема, которая приводила к сбою внедренных источников данных при перемещении отчетов.
...
- **Сервер отчетов Power BI**
    - *Версия 1.3.6816.37243 (сборка 15.0.2.557), дата выпуска: 30 августа 2018 г.*
        - Исправления ошибок
            - Устранена проблема, при которой во время обновления сервера с более ранних версий Сервера отчетов Power BI перенаправление привязки не обновлялось и клиенты видели такое сообщение:      
            *`
