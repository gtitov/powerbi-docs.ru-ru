...
| Набор данных Power BI | Единый вход | Наборы данных Power BI Premium и отличные от них. Требуется разрешение на чтение. Поддерживаются только режим импорта и наборы данных DirectQuery Power BI. |
| Набор данных Premium Power BI (XMLA) | Единый вход | Наборы данных Power BI не поддерживаются в качестве источников для внедренных отчетов с разбивкой на страницы в сценариях, когда "данные принадлежат приложению".  Чтобы обеспечить правильное подключение в Power BI Report Builder, убедитесь, что при настройке источника данных выбран параметр **Не использовать учетные данные**.<br />При доступе через XMLA учитывается членство в группе безопасности, заданное на уровне рабочей области или приложения.<br />Пользователи, у которых есть по крайней мере [роль участника в рабочей области](../collaborate-share/service-new-workspaces.md#roles-in-the-new-workspaces), могут визуализировать отчеты с разбивкой на страницы с помощью наборов данных Premium Power BI. Другим пользователям требуется [разрешение на сборку для базовых наборов данных](../connect-data/service-datasets-build-permissions.md).    |
| Введите данные | Н/Д | Данные внедряются в отчет. |
| Dataverse | SSO, OAuth2 | Невозможно использовать шлюз, так как многофакторная проверка подлинности (MFA) не поддерживается.
