...
В **Power BI Desktop** можно задать размер ячейки для числовых полей и полей времени. Можно создавать ячейки для вычисляемых столбцов, но не для мер. Используйте разделение на ячейки для объединения отображаемых в Power BI Desktop данных в группы необходимого размера.
...
Нажмите кнопку **ОК**. В области **Поля** появится новое поле с добавлением **(ячеек)**. Затем это поле можно перетащить на холст, чтобы использовать размер ячейки в визуальном элементе.
