...
Если вы используете [Premium на пользователя (PPU)](../admin/service-premium-per-user-faq.md), ваша рабочая область автоматически создается в емкости, связанной с PPU. В таких случаях разрешения емкости не требуются. Однако рабочие области, созданные пользователем PPU, могут быть доступны только другим пользователям PPU. Кроме того, содержимое, созданное в таких рабочих областях, может быть использовано только пользователями PPU.
...
* Пользователь с лицензией [Pro](../admin/service-admin-purchasing-power-bi-pro.md) или [PPU](../admin/service-premium-per-user-faq.md), который является членом обеих рабочих областей на исходном и целевом этапах развертывания.
...
## <a name="supported-dataset-features"></a>Поддерживаемые функции набора данных

Конвейеры развертывания поддерживают множество функций наборов данных Power BI. В этом разделе описаны две функции набора данных Power BI, которые могут улучшить работу с конвейерами развертывания:

* [Добавочное обновление](#incremental-refresh)

* [Составные модели](#composite-models)
...
### <a name="composite-models"></a>Составные модели

С помощью [составных моделей](../transform-model/desktop-composite-models.md) вы можете настроить отчет с несколькими подключениями данных.

Вы можете использовать функциональность составных моделей для подключения набора данных Power BI к внешнему набору данных, например к Azure Analysis Services. Дополнительные сведения см. в статье [Использование DirectQuery для наборов данных Power BI и Azure Analysis Services](../connect-data/desktop-directquery-datasets-azure-analysis-services.md).

В конвейере развертывания можно использовать составные модели для подключения набора данных к другому набору данных Power BI, который располагается за пределами конвейера.  

#### <a name="limitations"></a>Ограничения

Не поддерживаются следующие подключения составных моделей:

* связанные наборы данных, которые располагаются в одной рабочей области;

* связанные наборы данных, которые располагаются в разных конвейерах;

* связанные наборы данных, которые располагаются в одном конвейере. 
