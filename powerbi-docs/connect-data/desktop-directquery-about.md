...
> [!NOTE]
> Параметр максимального числа подключений DirectQuery применяется ко всем источникам DirectQuery, когда включены [улучшенные метаданные](desktop-enhanced-dataset-metadata.md), что является значением по умолчанию для всех моделей, созданных в Power BI Desktop, начиная с октября 2020 г. 
