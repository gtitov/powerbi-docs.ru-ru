---
title: Мониторинг метрик использования в классических рабочих областях
description: Просмотр, сохранение и применение метрик использования для панелей мониторинга и отчетов в классических рабочих областях.
author: maggiesMSFT
ms.author: maggies
ms.reviewer: ''
featuredvideoid: ''
...
ms.locfileid: "98861245"
---
# <a name="monitor-usage-metrics-in-classic-workspaces"></a>Мониторинг метрик использования в классических рабочих областях

Метрики использования помогают понять влияние панелей мониторинга и отчетов. Запустив метрики использования для отчета или панели мониторинга, вы узнаете, как эти панели мониторинга и отчеты используются в организации, а также что, кем и для какой цели используется. В этой статье описаны отчеты о метриках использования для [классических рабочих областей](service-create-workspaces.md). Если отчеты включены в новые рабочие области, ознакомьтесь со статьей, посвященной [мониторингу метрик использования в новых рабочих областях](service-modern-usage-metrics.md)  
...
| Срез страницы отчета |нет |да |Если отчет содержит более одной страницы, можно разделить отчет по страницам, которые были просмотрены. "Пусто" означает, что страница отчета добавлена недавно (фактическое имя новой страницы появится в списке среза в течение 24 часов) или страницы отчета удалены. Параметр Blank (Пусто) охватывает ситуации такого типа. |
...
Более того, базовый набор данных включает сведения об использовании для всех панелей мониторинга и отчетов в рабочей области. Это открывает дополнительные возможности. Например, вы можете создать отчет, который сравнивает панели мониторинга в рабочей области на основе сведений об использовании. Или вы можете создать панель мониторинга на основе метрик использования для приложения Power BI, объединив данные об использовании всего содержимого, опубликованного в этом приложении.  Сведения об удалении фильтра и [просмотре всех метрик использования для рабочей области](#see-all-workspace-usage-metrics) см. ниже в этой статье.
...
* Метрики использования иногда могут завышать количество действий в ситуациях, когда клиент обновляется без необходимости отправлять запрос обратно в службу Power BI. Например, при переключении страниц отчетов не создается запрос на загрузку отчета на сервер, так как определение страницы уже находится в браузере.
...
### <a name="discrepancies-between-rest-apis-and-usage-metrics"></a>Несоответствия между REST API и метриками использования

В [REST API отчетов](/rest/api/power-bi/reports) и [REST API администрирования](/rest/api/power-bi/admin) Power BI также используются данные службы Power BI. По причинам, описанным в предыдущем разделе, количество отчетов из API-интерфейсов может отличаться от количества отчетов в метриках использования. Количество отчетов, полученных от API, не влияет на клиентские проблемы и должно считаться точным. Также обратите внимание, что API администрирования предоставляют текущее состояние развертывания Power BI развертывания с учетом того, что существует на момент запроса. В классическом отчете с метриками использования содержатся данные за 90 дней. Общее число представляет уникальные отчеты, просматриваемые за более длинный период. Если отчеты удаляются после просмотра, они не учитываются API администрирования, но будут учитываться в исторических данных, на основе которых создается отчет об использовании.

### <a name="classic-usage-metrics-are-not-supported-with-private-links"></a>Классические метрики использования не поддерживаются с приватными каналами 

Если в организации используются [приватные каналы](../admin/service-security-private-links.md), отчеты о классических метриках использования не будут содержать данные. В настоящее время существует ограничение при передаче сведений о клиентах по приватным каналам. 
...
**Вопрос.**    Похоже, данные не обновляются. Например, методы распределения не отображаются, страницы отчетов отсутствуют и т. д.
