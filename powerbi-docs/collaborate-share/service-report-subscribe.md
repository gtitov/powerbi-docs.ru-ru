...
![Отправка по электронной почте моментального снимка панели мониторинга](media/service-report-subscribe/power-bi-subscriptions-email.png)
...
6. По умолчанию датой начала подписки является дата ее создания. Вы можете выбрать дату окончания. Если не задать дату окончания, срок действия подписки автоматически истекает через год. Вы можете указать любую дату в будущем (до 9999 года) в любое время до истечения срока действия подписки. Когда будет достигнута дата окончания подписки, ее действие прекращается до повторного включения. Перед запланированной датой окончания подписки вы получите уведомление с запросом на продление.
    ...
    На снимке экрана ниже обратите внимание, что при подписке на отчет вы фактически подписываетесь на _страницу_ отчета. Чтобы подписаться на несколько страниц в отчете, можно выбрать **Вложение полного отчета как** или настроить новую подписку, выбрав **Добавить другую подписку**.

    ![Область подписки](media/service-report-subscribe/power-bi-email.png)
...
1. (Необязательно) Выберите, следует ли вложить не одну страницу отчета, а _весь отчет_. Выберите вариант с PDF или PowerPoint. Размер вложения не должен превышать 20 страниц и 25 МБ. Для вложения учитываются все метки конфиденциальности, применяемые к отчету.

1. Щелкните **Сохранить и закрыть**. Подписчики получают сообщение электронной почты и моментальные снимки страницы отчета или панели мониторинга с выбранной частотой и в указанное вами время. В общем можно создать до 24 подписок на отчет или панель мониторинга и указать уникальных получателей, значения времени и частоты для каждой подписки. Все подписки, для которых для отчета или панели мониторинга задана частота **После обновления данных**, будут отправлять сообщение только после первого запланированного обновления.
...
- Если вы не получаете сообщения по подписке, сделайте следующее.

    - Убедитесь, что имя участника-пользователя (UPN) может получать сообщения электронной почты.
    - Хотя у вас есть лицензия Power BI Pro, возможно, у вас нет лицензии Microsoft Exchange. Если она отсутствует, для учетной записи Azure Active Directory может быть не указан адрес электронной почты или запасной адрес электронной почты. В этом случае, несмотря на то, что подписка отображается, вы не получите сообщение.  Если администратор Power BI назначает адрес электронной почты, Power BI синхронизирует обновление при следующем входе и использует этот адрес электронной почты для подписки.
...
- Если у вас нет основного адреса электронной почты, но есть запасной, Power BI использует его для доставки подписки.
- Если вы подписываете внешних пользователей на отчет или панель мониторинга, они получат уведомление об этом сразу после того, как вы нажмете **Сохранить и закрыть** в области подписки. Это уведомление отправляется только внешним, а не внутренним пользователям, так как им требуется ссылка с приглашением для просмотра отчета или панели мониторинга. 
- Существует ограничение на количество подписчиков для одного отчета или панели мониторинга. По достижении 200 подписчиков могут возникнуть проблемы с доставкой. Чтобы сократить число подписчиков, используйте вместо них псевдонимы электронной почты. Каждый псевдоним электронной почты считается одним подписчиком.
...
- Что касается подписок на отчеты Power BI по электронной почте, если для набора данных действует безопасность на уровне строк (RLS), вы можете создать подписки для себя. Вы можете также подписывать других пользователей на отчет с RLS. Эти подписки будут созданы с помощью вашего контекста безопасности.
...
- Функция вложения полного отчета доступна для отчетов, отвечающих следующим трем условиям:

    - Они находятся в обновленной рабочей области с [Power BI Premium или Premium на пользователя](../admin/service-premium-what-is.md). 
    - Размер файла вложения не превышает 25 МБ.
    - Отчет содержит менее 20 страниц. 
    
    Если отчет не соответствует этим трем условиям, создать подписки с полными отчетами в виде вложений не удастся. Все существующие подписки с полными отчетами в виде вложений будут отключены, и вы получите сообщение электронной почты с объяснением этой ошибки.
    
## <a name="next-steps"></a>Дальнейшие действия

- [Отчеты с разбивкой на страницы в службе Power BI. Подписка себя и других пользователей](../consumer/paginated-reports-subscriptions.md)
- Остались вопросы? [Попробуйте задать вопрос в сообществе Power BI.](https://community.powerbi.com/)
- Прочитайте [запись блога Power BI о подписках электронной почты для панелей мониторинга](https://powerbi.microsoft.com/blog/introducing-dashboard-email-subscriptions-a-360-degree-view-of-your-business-in-your-inbox-every-day/)
