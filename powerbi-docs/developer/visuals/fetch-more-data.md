...
API `fetchMoreData` позволяет визуальным элементам Power BI обходить жесткие ограничения по просмотру данных в 30 тысячах строк. В новом выпуске API 3.4 функция `fetchMoreData` API расширена для поддержки нового подхода к загрузке фрагментов данных. В дополнение к существующему подходу, который объединяет все запрошенные фрагменты, API будет поддерживать загрузку только добавочных фрагментов данных.

Новый подход обеспечивает большую гибкость при загрузке дополнительных блоков данных в визуальный элемент. Чтобы повысить производительность, можно настроить размер блока в соответствии с конкретными требованиями.
